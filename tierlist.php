...
document.addEventListener("DOMContentLoaded", () => {
    <?php if (!$isFriendView): ?>
        const tiers = document.querySelectorAll(".tier-items, #unranked-items");

        tiers.forEach(tier => {
            new Sortable(tier, {
                group: "tiers", // все контейнеры в одной группе
                animation: 150,
                onEnd: function (evt) {
                    const item = evt.item;
                    ...
                    fetch("scr/update_tier.php", {
                        ...
                        if (!res.ok) console.error("Ошибка обновления tier");
                    });
                }
            });
        });
    <?php else: ?>
        // просмотр чужого тир-листа — перетаскивание запрещено
    <?php endif; ?>
});
</script>
